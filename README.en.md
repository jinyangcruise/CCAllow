...
Automatically click "Allow" / "Allow once" buttons in Claude Desktop via Windows UI Automation.
...
Uses PowerShell UIAutomation to monitor the Claude Desktop window. When an "Allow once" / "Allow for this time" button is detected, it's clicked automatically. Priority is given to UIA InvokePattern (no window activation needed), falling back to focus activation + SendKeys Ctrl+Enter.
